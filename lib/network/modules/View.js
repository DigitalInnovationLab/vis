...
class View {
   ...
   */
  fit(options = {nodes:[],canvasSize:600}, initialZoom = false ) {
    let range;
    let zoomLevel;
    ...
    if (initialZoom === true) {
      ...
      let factor = Math.min(this.canvas.frame.canvas.clientWidth / options.canvasSize, this.canvas.frame.canvas.clientHeight / options.canvasSize);
      zoomLevel *= factor;
    }
    else {
